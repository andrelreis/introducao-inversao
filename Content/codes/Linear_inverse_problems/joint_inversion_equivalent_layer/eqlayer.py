...
def sensitivity_mag(coordinates,sources,main_field,magnetization):
    '''
...
####################### Gravity layer ##############################
def kernelz(x,y,z,xs,ys,zs):
    '''
    Calculate the first derivative of a function 1/r in relation to z.
    ...
    output
    phi_z : numpy array - Values with the first derivatives at one point.
    '''
    assert xs.size == ys.size and ys.size == zs.size and xs.size == zs.size, \
    'All arrays must have the same size'
    r = np.sqrt((x - xs)**2 + (y - ys)**2 + (z - zs)**2)
...
def gz(coordinates,sources,point_masses):
    '''
    Calculate the vertical component of the gravity field produced by a layer

    input

    coordinates : numpy arrays - Cartesian coordinates (in m) of the observation points
    sources : numpy arrays - Cartesian coordinates of equivalent sources
    point_masses: numpy array - Vector composed by point masses

    return
    gz : numpy array - vertical component of the gravitational field produced by the layer
    '''
    ...
    for i in range(N):
        gi = kernelz(x[i],y[i],z[i],xs,ys,zs)
        gz[i] = np.dot(p.T,gi)

    gz *= GRAVITATIONAL_CONST*1e5
    return gz

def sensitivity_grav(coordinates,sources):
    '''
    Calculate the sensitivity matrix
